...
public class IHttp {
	...
	private void setRequestHeaders(HttpURLConnection con) {
		 ...
		 * String prof = System.getProperty("microedition.profiles"); int space
		 * = prof.indexOf(' ');
		 * 
			...
			if (this.CMNET_USE_HTTPS) {
				...
				try {
					...
					while (true) {
						 ...
						 * (status == HttpConnection.HTTP_MOVED_TEMP) || (status
					...
					/*
					 * if ((status == HttpConnection.HTTP_NOT_IMPLEMENTED) ||
					 * (status == HttpConnection.HTTP_VERSION) || (status ==
					 * HttpConnection.HTTP_INTERNAL_ERROR) || (status ==
					 * HttpConnection.HTTP_GATEWAY_TIMEOUT) || (status ==
					...
					if ((status == HttpURLConnection.HTTP_NOT_IMPLEMENTED)
						...
						reValue = br.readLine();
						/*
						 * buffer.setLength(0); // Get the length and process
						 * the data int len = (int) con.getContentLength();
						 * 
						 * if (len > 0) { byte[] data = new byte[len]; int
						 * actual = is.read(data); process(data);
						 * logger.debug("......1:"+new String(data)); } else {
						 * int ch;
						 * 
						 * while ((ch = is.read()) != -1) { process((byte) ch);
